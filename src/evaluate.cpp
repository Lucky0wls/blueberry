...
int mgEval(const Board& board) {
    ...
    score += pawnMgValue * (wP - bP);
    score += knightMgValue * (wN - bN);
    score += bishopMgValue * (wB - bB);
    score += rookMgValue * (wR - bR);
    score += queenMgValue * (wQ - bQ);

    score += kingMgTable[__builtin_ctzll(board.pieces[WHITE][KING])];
    score -= kingMgTable[reverseSq(__builtin_ctzll(board.pieces[BLACK][KING]))];
    
    while (wP_bb) {
        int sq = popSq(wP_bb);
        score += pawnMgTable[sq];
    }
    while (wN_bb) {
        int sq = popSq(wN_bb);
        score += knightMgTable[sq];
    }
    while (wB_bb) {
        int sq = popSq(wB_bb);
        score += bishopMgTable[sq];
    }
    while (wR_bb) {
        int sq = popSq(wR_bb);
        score += rookMgTable[sq];
    }
    while (wQ_bb) {
        int sq = popSq(wQ_bb);
        score += queenMgTable[sq];
    }
    while (bP_bb) {
        int sq = popSq(bP_bb);
        score -= pawnMgTable[reverseSq(sq)];
    }
    while (bN_bb) {
        int sq = popSq(bN_bb);
        score -= knightMgTable[reverseSq(sq)];
    }
    while (bB_bb) {
        int sq = popSq(bB_bb);
        score -= bishopMgTable[reverseSq(sq)];
    }
    while (bR_bb) {
        int sq = popSq(bR_bb);
        score -= rookMgTable[reverseSq(sq)];
    }
    while (bQ_bb) {
        int sq = popSq(bQ_bb);
        score -= queenMgTable[reverseSq(sq)];
    }

    return score;
}

int egEval(const Board& board) {
    int score = 0;
    ...
    score += pawnEgValue * (wP - bP);
    score += knightEgValue * (wN - bN);
    score += bishopEgValue * (wB - bB);
    score += rookEgValue * (wR - bR);
    score += queenEgValue * (wQ - bQ);

    score += kingEgTable[__builtin_ctzll(board.pieces[WHITE][KING])];
    score -= kingEgTable[reverseSq(__builtin_ctzll(board.pieces[BLACK][KING]))];
    
    while (wP_bb) {
        int sq = popSq(wP_bb);
        score += pawnEgTable[sq];
    }
    while (wN_bb) {
        int sq = popSq(wN_bb);
        score += knightEgTable[sq];
    }
    while (wB_bb) {
        int sq = popSq(wB_bb);
        score += bishopEgTable[sq];
    }
    while (wR_bb) {
        int sq = popSq(wR_bb);
        score += rookEgTable[sq];
    }
    while (wQ_bb) {
        int sq = popSq(wQ_bb);
        score += queenEgTable[sq];
    }
    while (bP_bb) {
        int sq = popSq(bP_bb);
        score -= pawnEgTable[reverseSq(sq)];
    }
    while (bN_bb) {
        int sq = popSq(bN_bb);
        score -= knightEgTable[reverseSq(sq)];
    }
    while (bB_bb) {
        int sq = popSq(bB_bb);
        score -= bishopEgTable[reverseSq(sq)];
    }
    while (bR_bb) {
        int sq = popSq(bR_bb);
        score -= rookEgTable[reverseSq(sq)];
    }
    while (bQ_bb) {
        int sq = popSq(bQ_bb);
        score -= queenEgTable[reverseSq(sq)];
    }
...
int evaluate(const Board& board) {
    int mgScore = mgEval(board);
    int egScore = egEval(board);

    int knights = count(board.pieces[WHITE][KNIGHT]) + count(board.pieces[BLACK][KNIGHT]);
    int bishops = count(board.pieces[WHITE][BISHOP]) + count(board.pieces[BLACK][BISHOP]);
    int rooks = count(board.pieces[WHITE][ROOK]) + count(board.pieces[BLACK][ROOK]);
    int queens = count(board.pieces[WHITE][QUEEN]) + count(board.pieces[BLACK][QUEEN]);
